...
DISA STIG for Red Hat Enterprise Linux 9 Id: xccdf_org.ssgproject.content_profile_sti

[source,ini,role=execute,subs=attributes+]
----
sudo oscap xccdf eval  --profile xccdf_org.ssgproject.content_profile_sti --results ~/Documents/results_file.xml> /usr/share/xml/scap/ssg/content/ssg-rhel9-ds.xml
----
